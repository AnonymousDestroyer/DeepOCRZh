...
def pyMuPDF_fitz(pdfPath, imagePath):
    ...
    # for pg in range(pdfDoc.pageCount):
        page = pdfDoc[pg]   # page.get_pixmap() # int(width) int(length) alpha=False dpi=96
        rotate = int(0)
        ...
        # get_page_images 检测不出来复杂图表
        images = page.get_images()
        print("len of detected images", len(images))
        for i, im in enumerate(images):
            if im.colorspace.name not in (fitz.csGRAY.name, fitz.csRGB.name):
                im.save("7-%s.pam"%i)
            else:
                im.save("7-%s.jpeg"%i)
